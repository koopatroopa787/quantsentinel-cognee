...
import { Q, AGENTS } from "./DesignTokens";
...
function StatusDot({ status }: { status: string }) {
  const map: Record<string, string> = { done: Q.green, error: Q.red, warning: Q.amber, running: Q.accent, planning: Q.blue };
...
export default function AgentTimeline({ steps, running }: { steps: AgentStep[]; running: boolean }) {
  ...
  return (
    <div style={{
      ...
      position: 'relative', backdropFilter: 'blur(18px)',
    }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', paddingTop: 20, borderBottom: `1px solid ${Q.border}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 9, background: Q.accentLt, border: `1px solid ${Q.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ padding: '16px 20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 0, minHeight: 200, maxHeight: 420 }}>
        {isEmpty ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, paddingTop: 40, paddingBottom: 40 }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: Q.cardInner, border: `1.5px dashed ${Q.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 32px ${Q.accentGlow}20` }}>
          ...
          <>
            {steps.map((step, i) => {
              const meta = AGENTS[step.agent] || { label: step.agent, color: Q.sub, icon: '·' };
              ...
              return (
                <div key={`${step.agent}-${i}`} style={{
                  ...
                  border: `1px solid ${isRunning ? meta.color + '60' : Q.borderFaint}`,
                  borderLeft: `3px solid ${meta.color}`,
                  borderRadius: 12, padding: '12px 14px', marginBottom: 8,
                  boxShadow: isRunning ? `0 0 20px ${meta.color}25` : Q.shadow,
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 5, flexWrap: 'wrap' }}>
                        ...
                        {isDone && <span style={{ fontSize: 10, color: Q.green, background: Q.greenBg, border: `1px solid ${Q.greenBd}`, borderRadius: Q.rpill, padding: '2px 7px', fontWeight: 700 }}>✓ done</span>}
                        {isError && <span style={{ fontSize: 10, color: Q.red, background: Q.redBg, border: `1px solid ${Q.redBd}`, borderRadius: Q.rpill, padding: '2px 7px', fontWeight: 700 }}>✗ error</span>}
                        {isWarn && <span style={{ fontSize: 10, color: Q.amber, background: Q.amberBg, border: `1px solid ${Q.amberBd}`, borderRadius: Q.rpill, padding: '2px 7px', fontWeight: 700 }}>⚠ timeout</span>}
                        {isRunning && step.startedAt && <ElapsedTimer startedAt={step.startedAt}/>}
                      </div>

                      <p style={{ fontSize: 11, color: isRunning ? Q.sub : Q.muted, lineHeight: 1.6, margin: 0 }}>
                        {step.heartbeat ?? step.message}
                      </p>
                      {isRunning && <ProgressBar color={meta.color}/>}
